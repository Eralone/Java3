...
public class MyServer {
    ...
    public void start() throws IOException, SQLException {
        ...
        try {
            ...
            BaseAuthService.getConnection().close(); //TODO Так же закрываем подключение к SQL
        }
        }
    ...
    public void broadCastMessage(String message, ClientHandler clientHandler, boolean isServerInfoMsg, boolean PrivMsg) throws IOException { //Сообщение о подключении в чат юзера (всех, кроме самого этого юзера)
        for (ClientHandler handler : client) {
            //TODO Массив, который вместо отправки в чат своего сообщения самому себе (дубликат), кидает это сообщение в историю сообщений, находящийся в файле
            if (handler == clientHandler) { //Если получатель совпадает с отправителем, то вместо отправки сообщения - сохрани его в файле HistoryMessege
                ...
                try(FileWriter writer = new FileWriter(historyMesFile,true)) {
                    ...
                    writer.write(clientHandler.getUsername() + " / " + simpleDateFormat.format(new Date()) + " / " + message + ";\n");
                }
